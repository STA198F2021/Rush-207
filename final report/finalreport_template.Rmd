...
heart_grouped <- heart_grouped %>%
  mutate(press_level = cut(RestingBP,
                          breaks = c(-Inf, 120, 130, 140, 180, Inf),
                          labels = c("Normal", "Elevated", "Hypertension 1", "Hypertension 2", "Hypertensive Crisis"),
                          right=FALSE))
```
...
sex_table <- heart_grouped %>%
  tabyl(ChestPainType, Sex) 

exer_table <- heart_grouped %>%
  tabyl(ChestPainType, ExerciseAngina)

RBP_table <- heart_grouped %>%
  tabyl(ChestPainType, press_level)

chol_table <- heart_grouped %>%
  tabyl(ChestPainType, chol_level)
```
